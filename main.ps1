...
#Run DAST Scan: create a new scan, or RESCAN an existing one when scan_id is provided
$global:executionId = ''
$scanMode = 'New scan'

if(-not [string]::IsNullOrEmpty($env:INPUT_SCAN_ID)){
  $scanMode = 'Rescan'
  $global:scanId = $env:INPUT_SCAN_ID
  Write-Host "scan_id input provided - triggering a RESCAN (new execution) of existing scan: $global:scanId"
